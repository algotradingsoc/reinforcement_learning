...
def load_data(
    ...
    if price_source in ["OM"]:
        ...
        for t in tickers:
            df1 = pd.read_csv(
                "/disk3/thomas/reinforcement_learning/Thomas/csvdata/OM/{}.csv".format(
                    t
                )
            )
...
def train_EQ_env(config):
    ...
    agent = SACTrainer(config["trainer"], Equitydaily)
    best_reward = -np.inf
    ...
    while best_reward < config["best_reward"]:
        ...
        while not done:
            ...
            state, reward, done, future_price = testenv.step(action)
            cum_reward += reward
...
if __name__ == "__main__":
    ...
    testconfig = dict()
    testconfig["trainer"] = config.copy()
    ...
    checkpoints = analysis.get_trial_checkpoints_paths(
        trial=analysis.get_best_trial("reward"),
        metric="reward",
    )
